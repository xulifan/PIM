...
__kernel void BFS_1_pim( const __global Node* g_graph_nodes,
					const __global int* g_graph_edges, 
					__global char* g_graph_mask, 
					__global char* g_updating_graph_mask, 
					__global char* g_graph_visited, 
                    ...
                    const int end_node,
                    const int own_num_nodes){
	int tid = get_global_id(0);
    
if(tid<own_num_nodes){
    tid+=start_node;
    if( tid<no_of_nodes && g_graph_mask[tid]){
		g_graph_mask[tid]=false;
		for(int i=g_graph_nodes[tid].starting; i<(g_graph_nodes[tid].no_of_edges + g_graph_nodes[tid].starting); i++){
...
__kernel void BFS_2_pim(__global char* g_graph_mask, 
					__global char* g_updating_graph_mask, 
					__global char* g_graph_visited, 
					__global char* g_over,
                    __global int* g_cost,
                    ...
                    const int own_num_nodes,
                    const int level){
	int tid = get_global_id(0)+start_node;
	if( tid<no_of_nodes && g_updating_graph_mask[tid]){
        g_cost[tid]=level;
		g_graph_mask[tid]=true;
		g_graph_visited[tid]=true;
