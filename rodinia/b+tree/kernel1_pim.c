...
/******************** PIM meory allocate ****************************/
    ...
    pim_ans=(void **)malloc(sizeof(void *)*num_gpus);
    ...
    pim_mapped_ans=(record **)malloc(sizeof(record *)*num_gpus);
...
/****************** Allocate Memory on each PIM *********************/
    ...
    for(int cur_gpu=0;cur_gpu<num_gpus;cur_gpu++){
        ...
        pim_ans[cur_gpu] = pim_malloc(count*sizeof(record), target_gpu[cur_gpu], PIM_MEM_PIM_READ | PIM_MEM_HOST_RW, PIM_PLATFORM_OPENCL_GPU);
    }
...
/****************** Copy Memory to each PIM *************************/
    ...
    for(int cur_gpu=0;cur_gpu<num_gpus;cur_gpu++){
        ...
        pim_mapped_keys[cur_gpu] = (int *)pim_map(pim_keys[cur_gpu],PIM_PLATFORM_OPENCL_GPU);
...
/****************** Launch kernels on each PIM **********************/
    ...
    for(int cur_gpu=0;cur_gpu<num_gpus;cur_gpu++){
        pim_launch_findK_kernel(pim_knodes[cur_gpu], pim_records[cur_gpu], pim_currKnode[cur_gpu], pim_offset[cur_gpu], pim_keys[cur_gpu], pim_ans[cur_gpu], maxheight, knodes_elem, start_point[cur_gpu], order, own_num_points[cur_gpu], target_gpu[cur_gpu], &complete_event[cur_gpu]);
    }
...
/******************** collect resutls from PIMs *********************/
    ...
    for(int cur_gpu=0;cur_gpu<num_gpus;cur_gpu++){  
        pim_mapped_ans[cur_gpu] = (record *)pim_map(pim_ans[cur_gpu],PIM_PLATFORM_OPENCL_GPU);
        memcpy(&ans[start_point[cur_gpu]],&pim_mapped_ans[cur_gpu][start_point[cur_gpu]],sizeof(record)*own_num_points[cur_gpu]);
        pim_unmap(pim_mapped_ans[cur_gpu]);
    }
