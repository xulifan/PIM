...
int main(int argc, char** argv) {
	...
	if (error != CL_SUCCESS) fatal_CL(error, __LINE__);
	printf("Cols :%d Rows %d gridcol %d gridrow %d smallcol %d smallrow %d block_size %d pyramid %d\n",blockCols,blockRows,grid_cols,grid_rows,smallBlockCol,smallBlockRow,BLOCK_SIZE,pyramid_height);	
    ...
    float *MatrixOut=compute_tran_temp_pim(MatrixPower, MatrixTemp, grid_cols, grid_rows, total_iterations, pyramid_height,
								blockCols, blockRows, borderCols, borderRows, FilesavingTemp, FilesavingPower);
