...
__kernel void likelihood_kernel_pim(__global float * arrayX, __global float * arrayY,__global float * xj, __global float * yj, __global float * CDF, __global int * ind, __global int * objxy, __global float * likelihood, __global unsigned char * I, __global float * u, __global float * weights, const int Nparticles, const int countOnes, const int max_size, int k, const int IszY, const int Nfr, __global int *seed, __global float * partial_sums, __local float* buffer,int start_point){
	int block_id = get_group_id(0);
	int thread_id = get_local_id(0);
...
__kernel void normalize_weights_kernel_pim(__global float * weights, int Nparticles, __global float * CDF, __global float * u, __global int * seed, float partial_sums, int start_point)
{
	int i = get_global_id(0)+start_point;
	int local_id = get_local_id(0);
	__local float u1;
...
__kernel void find_index_kernel_pim(__global float * arrayX, __global float * arrayY, 
	__global float * CDF, __global float * u, __global float * xj, 
	__global float * yj, __global float * weights, int Nparticles, int start_point
	){
		int i = get_global_id(0)+start_point;
