...
__kernel void likelihood_kernel_pim(__global double * arrayX, __global double * arrayY,__global double * xj, __global double * yj, __global double * CDF, __global int * ind, __global int * objxy, __global double * likelihood, __global unsigned char * I, __global double * u, __global double * weights, const int Nparticles, const int countOnes, const int max_size, int k, const int IszY, const int Nfr, __global int *seed, __global double * partial_sums, __local double* buffer,int start_point, int own_num_points){
	int block_id = get_group_id(0);
	int thread_id = get_local_id(0);
	int i = get_global_id(0)+start_point;
        size_t THREADS_PER_BLOCK = get_local_size(0);
	int y;
	int indX, indY;
	
	
	if(i < Nparticles && get_global_id(0) < own_num_points){
		arrayX[i] = xj[i]; 
		arrayY[i] = yj[i]; 

		weights[i] =100;// 1 / ((double) (Nparticles)); //Donnie - moved this line from end of find_index_kernel to prevent all weights from being reset before calculating position on final iteration.
...
__kernel void normalize_weights_kernel_pim(__global double * weights, int Nparticles, __global double * CDF, __global double * u, __global int * seed, double partial_sums, int start_point)
{
	int i = get_global_id(0)+start_point;
	int local_id = get_local_id(0);
	__local double u1;
...
__kernel void find_index_kernel_pim(__global double * arrayX, __global double * arrayY, 
	__global double * CDF, __global double * u, __global double * xj, 
	__global double * yj, __global double * weights, int Nparticles,int start_point
	){
		int i = get_global_id(0)+start_point;
