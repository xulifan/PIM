...
/******************** PIM meory variable ****************************/
    ...
    double **pim_mapped_arrayX;
    double **pim_mapped_arrayY;
    double **pim_mapped_xj;
    double **pim_mapped_yj;
    double **pim_mapped_CDF;
    double **pim_mapped_u;
    double **pim_mapped_likelihood;
    double **pim_mapped_weights;
    unsigned char **pim_mapped_I;
    int **pim_mapped_objxy;
    int **pim_mapped_ind;
    int **pim_mapped_seed;
    double **pim_mapped_partial_sums;
...
/******************** PIM meory allocate ****************************/
    ...
    pim_mapped_arrayX=(double **)malloc(sizeof(double *)*num_gpus);
    pim_mapped_arrayY=(double **)malloc(sizeof(double *)*num_gpus);
    pim_mapped_xj=(double **)malloc(sizeof(double *)*num_gpus);
    pim_mapped_yj=(double **)malloc(sizeof(double *)*num_gpus);
    pim_mapped_CDF=(double **)malloc(sizeof(double *)*num_gpus);
    pim_mapped_u=(double **)malloc(sizeof(double *)*num_gpus);
    pim_mapped_likelihood=(double **)malloc(sizeof(double *)*num_gpus);
    pim_mapped_weights=(double **)malloc(sizeof(double *)*num_gpus);
    ...
    pim_mapped_partial_sums=(double **)malloc(sizeof(double *)*num_gpus);
...
/****************** Allocate Memory on each PIM *********************/
    ...
    for(int cur_gpu=0;cur_gpu<num_gpus;cur_gpu++){
        pim_arrayX[cur_gpu] = pim_malloc(sizeof (double) *Nparticles, target_gpu[cur_gpu], PIM_MEM_PIM_RW | PIM_MEM_HOST_RW, PIM_PLATFORM_OPENCL_GPU);
        pim_arrayY[cur_gpu] = pim_malloc(sizeof (double) *Nparticles, target_gpu[cur_gpu], PIM_MEM_PIM_RW | PIM_MEM_HOST_RW, PIM_PLATFORM_OPENCL_GPU);
        pim_xj[cur_gpu] = pim_malloc(sizeof (double) *Nparticles, target_gpu[cur_gpu], PIM_MEM_PIM_RW | PIM_MEM_HOST_RW, PIM_PLATFORM_OPENCL_GPU);
        pim_yj[cur_gpu] = pim_malloc(sizeof (double) *Nparticles, target_gpu[cur_gpu], PIM_MEM_PIM_RW | PIM_MEM_HOST_RW, PIM_PLATFORM_OPENCL_GPU);
        pim_CDF[cur_gpu] = pim_malloc(sizeof (double) *Nparticles, target_gpu[cur_gpu], PIM_MEM_PIM_RW | PIM_MEM_HOST_RW, PIM_PLATFORM_OPENCL_GPU);
        pim_u[cur_gpu] = pim_malloc(sizeof (double) *Nparticles, target_gpu[cur_gpu], PIM_MEM_PIM_RW | PIM_MEM_HOST_RW, PIM_PLATFORM_OPENCL_GPU);
        pim_likelihood[cur_gpu] = pim_malloc(sizeof (double) *Nparticles, target_gpu[cur_gpu], PIM_MEM_PIM_RW | PIM_MEM_HOST_RW, PIM_PLATFORM_OPENCL_GPU);
        pim_weights[cur_gpu] = pim_malloc(sizeof (double) *Nparticles, target_gpu[cur_gpu], PIM_MEM_PIM_RW | PIM_MEM_HOST_RW, PIM_PLATFORM_OPENCL_GPU);
        ...
        pim_partial_sums[cur_gpu] = pim_malloc(sizeof (double) * Nparticles, target_gpu[cur_gpu], PIM_MEM_PIM_RW | PIM_MEM_HOST_RW, PIM_PLATFORM_OPENCL_GPU);
    }
...
/****************** Copy Memory to each PIM *************************/
    ...
    double *partial_sums_cpu=(double *)calloc(Nparticles,sizeof (double));
    //for(int i=0;i<Nparticles;i++) printf("%lf ",weights[i]);
    //printf("\n");
...
/****************** Launch kernels on each PIM **********************/
    ...
    for (k = 1; k < Nfr; k++) {
        ...
        for(int cur_gpu=0;cur_gpu<num_gpus;cur_gpu++){
            pim_memcpyHtoD(partial_sums_cpu,pim_partial_sums[cur_gpu],sizeof (double) *Nparticles);
        }
        ...
        double sum=0;
        for(int cur_gpu=0;cur_gpu<num_gpus;cur_gpu++){
            pim_mapped_partial_sums[cur_gpu]=(double *)pim_map(pim_partial_sums[cur_gpu],PIM_PLATFORM_OPENCL_GPU);
        ...
        for(int cur_gpu=0;cur_gpu<num_gpus;cur_gpu++){
            //printf("cur_gpu %d\n",cur_gpu);
            pim_mapped_weights[cur_gpu]=(double *)pim_map(pim_weights[cur_gpu],PIM_PLATFORM_OPENCL_GPU);
            //for(int i=0;i<Nparticles;i++) printf("%lf ",pim_mapped_weights[cur_gpu][i]);
            //printf("\n");
            pim_unmap(pim_mapped_weights[cur_gpu]);
        }
...
{
    char * source_nm = (char *)"particle_double.cl";
...
void pim_launch_normalize_weights_kernel(void *weights, int Nparticles, void *CDF, void *u, void *seed, double partial_sum, int start_point, int own_num_points, pim_device_id target, cl_event *complete)
{
    char * source_nm = (char *)"particle_double.cl";
