...
#if 0
void initialize_pim(char *input, char *file)
{
    ...
    filename = concat_directory_and_name(directory, file);
...
void *parallel_prefix_sum(void *args)
{
    ...
    PRINT_DEBUG( "Thread 0x%x started\n", ( int ) pthread_self(  ) );
...
/*
	...
	PRINT_DEBUG("Using default specs in cpu_model.xml\n");
	file="cpu_model.xml";
    }	
*/
    ...
    PRINT_DEBUG("Number of threads: %d\n", num_threads);
    ...
    PRINT_DEBUG("Generating initial array...");
    fflush(stdout);


    for (i = 0; i < num_threads; i++) {
        for (j = 0; j < amount_to_move; j++) {
            parallel_input_array[i][j] = i*j+4;
            parallel_output_array[i][j] = 0;
        }
    }

    PRINT_DEBUG("Calculating serial parallel prefix...\n");
    fflush(stdout);



    PRINT_DEBUG("Parallel prefix stage 1...");
    fflush(stdout);
    ...
    for (i = 0; i < num_threads; i++) {
     ...
     // ***** Exit Mark 4 end of program **
     //
	 	pim_emu_end();
 

     PRINT_DEBUG("Checking results.\n");
    ...
    for (i = 0; i < num_threads; i++) {
        for (j = 0; j < amount_to_move; j++) {
            input_array[(i*amount_to_move)+j]=parallel_input_array[i][j];
            output_array[(i*amount_to_move)+j] = 0;
        }
    }
